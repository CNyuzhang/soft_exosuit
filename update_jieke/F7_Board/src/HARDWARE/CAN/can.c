...
u8 CAN1_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
	...
	MY_NVIC_Init(1,3,CAN1_RX0_IRQn,2);//组2
#endif
	return 0;
}   
...
void CAN1_RX0_IRQHandler(void)
{
	...
	pmsg.id=id;pmsg.ide=ide;pmsg.rtr=rtr;pmsg.len=len;
	for(u8 i=0;i<8;i++){pmsg.data[i]=rxbuf[i];}
...
//    printf("rxbuf[7]:%d\r\n",rxbuf[7]);
	...
	copley_node1.update(&pmsg);
	copley_node2.update(&pmsg);
...
//返回值:0,成功;
//		 其他,失败;
u8 CAN1_Send_Msg(struct can_msg *buf,u8 len)
{	
	u8 mbox;
	u16 i=0;	 
  struct can_msg *pmsg = (struct can_msg *) buf;
	
  //mbox=CAN1_Tx_Msg(0X12,0,0,len,msg);
	mbox=CAN1_Tx_Msg(pmsg->id,pmsg->ide,pmsg->rtr,len,pmsg->data);//CAN1_Tx_Msg(u32 id,u8 ide,u8 rtr,u8 len,u8 *dat)
